...
| 2026-01-26 | Tasks 5.62-5.63: V2.5 bug fixes | Sonnet | Fixed SOQL builder parent lookup filtering bug. Added isChildRelationship() check in buildDynamicSOQL() to filter out child relationships (Contact, Opportunity) from parent lookup processing. Only true parent lookups (Owner, Account) added to rootFields. Fixed PromptBuilder to use Execution RecordType with Type='Text' instead of Builder RecordType with 'Context Template'. Deployed to agentictso. Commit a307943. |
